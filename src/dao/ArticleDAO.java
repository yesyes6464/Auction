...
public class ArticleDAO {
	...
	public int insertArticle(ArticleBean articleBean,int Auc_time) {
		...
		try {
			...
			pstmt = con.prepareStatement("SELECT DATE_ADD(NOW(), INTERVAL ? HOUR)");
			pstmt.setInt(1,	Auc_time);
			rs = pstmt.executeQuery();
	...
	public ArrayList<ArticleBean> selectArticleList(int page, int limit){
		...
		try {
			...
			while(rs.next()){
				...
				article.setAuction_time(rs.getDate("auction_time"));
	...
	public ArticleBean articleDetail(int num) {
		...
		try {
			...
			if(rs.next()){
				...
				bean.setAuction_time(rs.getDate("auction_time"));
		...
		ArticleBean abn = null;
		 try {
	         ...
	         while (rs.next()) {
	            ...
	            abn.setAuction_time(rs.getDate("auction_time"));
